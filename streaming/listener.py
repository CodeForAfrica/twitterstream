...
from twitterstream.config import TW_AUTH_CREDENTIALS, GSPREAD_CONFIG, CACHE_HOST, USER_CAP, DEFAULT_SPREADSHEET_ID
...
def main(sheet_id, stream_type, filters=[]):
    '''
    ...
    try:
        ...
        print "Creating logger... %s - %s - %s" % (sheet_id, stream_type, filters)
        logging.basicConfig(filename=LOGGING['location'] % stream_type,
                level=LOGGING['level'],
                format=LOGGING['format'])
        logging.info("Starting %s stream against %s [ %s ]" % (stream_type, filters, sheet_id))


        stream = Streamer(sheet_id)
        if stream_type == "user":
...
class Streamer(object):
    ...
    def start_user_stream(self, users=[]):
        ...
        '''
        tweepy.Stream(get_api(auth_only=True), self.stream_listener).filter(
    ...
    def start_hashtag_stream(self, hashtags=[]):
        ...
        '''
        tweepy.Stream(get_api(auth_only=True), self.stream_listener).filter(track=hashtags)
        #self.logger.info("Stream %s started." % self.stream_id)
